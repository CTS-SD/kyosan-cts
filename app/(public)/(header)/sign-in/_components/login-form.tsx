...
export const LoginForm = ({ onSignOut, userName }: { onSignOut: () => void; userName?: string }) => {
        ...
        <div className="rounded-4xl bg-surface shadow-lg ring-4 ring-background">
          ...
          {userName && (
            <div className="flex gap-2 p-4 text-sm">
              <InfoIcon className="h-lh text-muted-foreground" />
              <div className="">
                あなたは「<span className="font-semibold">{userName}</span>」としてログインしています。
                <button className="text-muted-foreground underline" onClick={onSignOut}>
                  ログアウト
                </button>
              </div>
            </div>

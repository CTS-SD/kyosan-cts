...
export const QuizFormSelect = () => {
  ...
  const correctKey = quiz.params.correctChoices.join("\n");
  const incorrectKey = quiz.params.incorrectChoices.join("\n");

  // biome-ignore lint/correctness/useExhaustiveDependencies: The dependencies are correct.
  const choices = useMemo(() => {
    return shuffle(
      [...quiz.params.correctChoices, ...quiz.params.incorrectChoices].map((choice) => choice.trim()).filter(Boolean),
    );
  ...
  const handleChoice = (choice: string) => {
    ...
    if (selections.length === quiz.params.correctChoices.length) {
    ...
    <div className="px-4">
      <div className="relative flex flex-col gap-2.5">
        {choices.map((choice, index) => {
          ...
          const isHighlighted = result?.isCorrect && isSelected && quiz.params.correctChoices.includes(choice);

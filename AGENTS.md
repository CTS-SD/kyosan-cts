...
- **E2E** (Playwright): specs in `playwright/` (`public/`, `members/`, `admin/`, `auth/`), runs serially (1 worker) against a prod build on `:3001` using `.env.test`. Two projects: `chromium` (everything) and `mobile-chrome` (Pixel 5, only `public/**` + `members/**`; admin tooling is desktop-only). Locally the web server runs `pnpm build && pnpm start`; in CI it builds in a separate step and just starts. The auto `_db` fixture truncates + reseeds the test DB before every test (`playwright/helpers/db.ts`), seeding both an `admin` and a `member` account. `adminPage` / `memberPage` are pre-authenticated pages that log in via `/api/auth/test-login?role=…` (a route that only responds when `PLAYWRIGHT_TEST=1`). Use `setTestConfig(key, value)` to set a config row for a test (e.g. publish department announcements). `global-setup.ts` drops all tables and re-migrates once before the run.
